        ...
        try
        {
            using (NpgsqlConnection connection = new NpgsqlConnection(connectionString))
            {
                ...
                string commandString = string.Format("SELECT * from tasks " +
                                                     "WHERE advertising_projects_id='{0}'", projectId);
                    ...
                    using (NpgsqlDataReader reader = command.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            tasksList.Add(new Tasks(
                                ...
                                new TaskStatusesDAO().getStatus(reader.GetFieldValue<int>(4)),
                                reader.GetFieldValue<string>(5),
                                new ProjectDAO().getProject(reader.GetFieldValue<long>(6))
                                ));
                        }
                        reader.Close();
                    }
                ...
                using (NpgsqlCommand command=new NpgsqlCommand(commandString,connection))
                {
                    using (NpgsqlDataReader reader=command.ExecuteReader())
                    {
                        if (reader.Read())
                        {
                            ...
                            task.project = new ProjectDAO().getProject(reader.GetFieldValue<long>(6));
                        }
                        reader.Close();
                    }
                }
        ...
        try
        {
            using (NpgsqlConnection connection = new NpgsqlConnection(connectionString))
            {
                ...
                String commandString = String.Format(
                    "INSERT INTO tasks (title, description, deadline, status_id, assigned_to, advertising_projects_id) " +
                    "VALUES('{0}','{1}','{2}','{3}','{4}','{5}')",
                    task.title, task.description, task.deadline.ToString("yyyy-MM-dd"), task.status.id,
                    task.assignedTo, task.project.id);
        ...
        try
        {
            using (NpgsqlConnection connection = new NpgsqlConnection(connectionString))
            {
                ...
                String commandString = String.Format("UPDATE tasks " +
                                                     ...
                                                     "status_id='{3}'," +
                                                     "assigned_to='{4}'," +
                                                     "advertising_projects_id='{5}' " +
                                                     "WHERE id='{6}'",
                    task.title, task.description, task.deadline.ToString("yyyy-MM-dd"),
                    task.status.id, task.assignedTo, task.project.id, id);
                using (NpgsqlCommand command = new NpgsqlCommand(commandString, connection))
                {
                    command.ExecuteNonQuery();
                }
